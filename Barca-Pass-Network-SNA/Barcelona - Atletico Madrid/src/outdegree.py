...
file_path = "C:/Users/hanza/Desktop/social/week3/pas_count_summary.xlsx"
...
player_positions = {
    ...
    'Ivan Rakitić': (2.5, 3),   # Orta Saha 1
    'Sergio Busquets i Burgos': (2.1, 5),   # Orta Saha 2
    'Nélson Cabral Semedo': (1.9, 4.5),   # Orta Saha 1
    'Arthur Henrique Ramos de Oliveira Melo': (2.5, 7),   # Orta Saha 3
    'Arturo Erasmo Vidal Pardo': (2.2, 6.5),   # Orta Saha 3
...
# Out-degree centrality hesaplama
out_degree_centrality = nx.out_degree_centrality(G)

# Out-degree centrality değerlerini listeye çevirme
out_degree_values = list(out_degree_centrality.values())

# Renk skalası için normalizasyon
norm = plt.Normalize(vmin=min(out_degree_values), vmax=max(out_degree_values))
...
# Out-degree değerlerine göre renk haritası
out_color_map = plt.cm.Reds(norm(out_degree_values))
...
nx.draw(
    ...
    node_color=out_color_map,  # Renk haritası
...
# Out-degree centrality renk skalasını ekleyelim
ax = plt.gca()  
plt.colorbar(plt.cm.ScalarMappable(cmap=plt.cm.Reds, norm=norm), ax=ax, label="Out-Degree Centrality")

# Başlık ekle
plt.title("4-3-3 Pas Ağı ve Out-Degree Merkeziği", fontsize=16)
plt.show()
