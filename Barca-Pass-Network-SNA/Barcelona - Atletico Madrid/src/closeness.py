...
# Closeness centrality değerlerini listeye çevirme
...
# Closeness centrality değerlerine göre renk haritası (mor tonları)
closeness_color_map = plt.cm.Purples(norm(closeness_values))
...
nx.draw(
    ...
    node_color=closeness_color_map,  # Renk haritası
...
# Closeness centrality renk skalasını ekleyelim
ax = plt.gca()  
plt.colorbar(plt.cm.ScalarMappable(cmap=plt.cm.Purples, norm=norm), ax=ax, label="Closeness Centrality")

# Başlık ekle
plt.title("4-3-3 Pas Ağı ve Closeness Merkeziği (Mor Renk Skalası)", fontsize=16)
plt.show()
